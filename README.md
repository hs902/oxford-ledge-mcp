...
> **Version:** 3.7.1 (gov-public-data-only surface; CUSIP + third-party-FRED carve-outs)
...
| `get_insider_trades` | A | Form 4 transactions for one issuer — latest 20, every code, derivative rows labelled, a company filing under its own SEC ID labelled `issuerSelfFiled` (leave those out of totals), `dateBasis` names the served date |
| `ol_insider_recent_buys` | A | Market-wide OPEN-MARKET insider purchases across the Oxford Ledge issuer catalog, newest first — a daily insider screen (Form 4; `totalValue`/`position` are Oxford Ledge derivations; issuer-self-filed purchases excluded) |
...
| `get_bdc_list` | A | Active tracked BDCs with `totalFairValue` (arbitrated — read `fairValueBasis` and `fairValueGap`), holding counts and filing dates (Oxford Ledge parse) |
| `get_bdc_holdings` | A | One BDC's full latest-filing portfolio: borrower, industry, security type, lien, rate, maturity, par/cost/fair value (USD) and mark (percent of par) per position, arbitrated totals with the `fairValueGap` label (SEC SoI; Oxford Ledge parse) |
...
| `ol_bdc_top_borrowers` | A | Borrowers syndicated across the MOST BDCs, ranked by ACTIVE lender count then exposure — the private-credit discovery entrypoint (SEC SoI; Oxford Ledge parse; free) |
...
| `ol_bdc_credit_quality` | A | Non-accrual share and its trend for one BDC, withheld below 90% determinate coverage or when more than 25% is flagged on loans marked near par (the misread signature; `coverage_state` says which), stated but labelled `unusually_high` when more than 25% is flagged on loans marked below 85 or with no mark on file (SEC SoI; Oxford Ledge parse) |
...
| Tool | Mode | Description |
|------|:-:|-------------|
| `get_value_investing_fact` | A | Buffett / Graham / Munger principles and attributed paraphrases from Oxford Ledge's curated corpus, with an `attribution` credit line — wording not verified against the source, so not verbatim quotations unless `verbatim` is true (ol-authored) |
...
- **Oxford Ledge proprietary parses** — BDC holdings from SEC SoI filings (48 active BDCs on `get_bdc_list` at the 3.7.1 cut; the final filings of wound-down or merged-away BDCs stay readable through the borrower tools, labelled inactive) + the curated value-investing corpus. API mode only; their payloads say `ol-derived` / `hybrid` / `ol-authored` in `_meta.basis`, and their descriptions say it in prose.
